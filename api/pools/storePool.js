...
function createPoolMetadata(chainID, poolID) {
  ...
  var metadata = {
    ...
    "external_url": `https://analytics.hydrogen.hysland.finance/pools/?chainID=${chainID}&poolID=${poolID}`,
...
async function storePool(chainID, nucleusAddress, poolID) {
  ...
  await Promise.all([
    ...
    s3PutObjectPromise({ Bucket: "assets.hysland.finance", Key: `hydrogen/hpt/${chainID}/${poolID}.svg`, Body: svg, ContentType: "image/svg+xml"})
  ])
}
